...
class Config(object):
    ...
    def use_pretrained_embeddings(self, flag):
        self.pretrained_embeddings = flag
    
    def set_pretrain_embedding_path(self, path_to_file):
        self.pretrained_embeddings_path = path_to_file
    ...
    def train_one_step(self):
        self.trainModel.batch_h = to_var(self.batch_h, self.cuda)
        self.trainModel.batch_t = to_var(self.batch_t, self.cuda)
        self.trainModel.batch_r = to_var(self.batch_r, self.cuda)
        self.trainModel.batch_y = to_var(self.batch_y, self.cuda)
    ...
    def test_one_step(self, model, test_h, test_t, test_r):
        model.batch_h = to_var(test_h, self.cuda)
        model.batch_t = to_var(test_t, self.cuda)
        model.batch_r = to_var(test_r, self.cuda)
    ...
    def train(self):
        ...
        for epoch in range(self.train_times):
            ...
            # Method -3 - Best Model is with Min loss
            if res < min_loss:
                min_loss = res
                best_epoch = epoch
            ...
            if False and (epoch + 1) % self.valid_steps == 0:
        ...
        print("Store model & embeddings of best result at epoch %d..." % (best_epoch))
